...
extension MainViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(withIdentifier: CustomPokemonCell.identifier, for: indexPath) as? CustomPokemonCell else {
            fatalError("couldnt dequeue")
        }
        
        let item = pokemons[indexPath.row]
        cell.configure(with: item.url, and: item.name)
